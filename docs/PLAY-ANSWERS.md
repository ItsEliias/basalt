...
| Health & fitness | Health info — **mental health** | Yes, optional | App functionality | V4 Wellbeing: daily mood/energy/stress words (1–5), optional check-in note, optional journal (LOCAL-ONLY by default — cloud sync is a separate opt-in switch). Never scored, never diagnosed, no screening instruments. Crisis-detection phrase matching runs entirely on-device; crisis resources are always one tap away and nothing about a crisis leaves the device except the user's own call/text |
